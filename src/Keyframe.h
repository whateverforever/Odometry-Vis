// The file contains declaration of keyframe. Whenever a keyframe is selected, a
// corresponding object is created. Therefore, the keyframe object can only be
// created through dynamic memory allocator Created by Yu Wang on 2019-01-13.
#pragma once
...
public:
  ...
  KeyFrame(const std::shared_ptr<cv::Mat> &kLeftImg,
           const std::shared_ptr<cv::Mat> &kRightImg,
           const std::shared_ptr<cv::Mat> &kLeftDep,
           const std::shared_ptr<cv::Mat> &kLeftVal, const Affine4f kAbsoPose);
   ...
   * ***************************/
  // The depth values, depth valid mask, absolute pose might change during
  // global optimization
...
private:
  /*** Shared Mem pointers, the corresponding memories will always be valid
